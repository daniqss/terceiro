...
class Model:
    ...
    def get_patients(self) -> List[dict]:
        ...
        if status != 200:
            raise DataErrorException(DataType.PATIENT, OperationType.GET)
        return patients
    ...
    def get_medications(self, patient_id: int) -> Optional[List[dict]]:
        ...
        if status != 200:
            raise DataErrorException(DataType.MEDICATION, OperationType.GET)
        return medications
    ...
    def delete_medication(self, patient_id: int, medication_id: int):
        ...
        if status != 204:
            raise DataErrorException(DataType.MEDICATION, OperationType.DELETE)
    ...
    def add_medication(
        ...
        if status != 201:
            raise DataErrorException(DataType.MEDICATION, OperationType.POST)
        return response
    ...
    def update_medication(
        ...
        if status != 204:
            raise DataErrorException(DataType.MEDICATION, OperationType.PATCH)
        return response
    ...
    def get_posologies(self, patient_id: int, medication_id: int) -> Optional[List[dict]]:
        ...
        if status != 200:
            raise DataErrorException(DataType.POSOLOGY, OperationType.GET)
        return posologies
    ...
    def delete_posology(self, patient_id: int, medication_id: int, posology_id: int) -> bool:
        ...
        if status != 204:
            raise DataErrorException(DataType.POSOLOGY, OperationType.DELETE)
        return response
    ...
    def add_posology(self, patient_id: int, medication_id: int, minute:int, hour:int) -> bool:
        ...
        if status != 201:
            raise DataErrorException(DataType.POSOLOGY, OperationType.POST)
        return response
    ...
    def update_posology(self, patient_id: int, medication_id: int, posology_id:int, minute:int, hour:int):
        ...
        if status != 204:
            raise DataErrorException(DataType.POSOLOGY, OperationType.PATCH)
        return response

export class PatientListView {
    ...
    renderSuggestions(patients) {
      if (!patients || patients.length === 0) {
        this.suggestionsContainer.innerHTML = "<li class='error'>No se encontraron resultados.</li>";
        return;
      }
  ...
  export class MedicationView {
    constructor(container, scheduleBody, averageTimeElement, totalIntakesElement) {
          ...
          (med) => `
            <li>
              <article class="medication-item">
                <h3>
                  <a href="./src/ibupro.html?patientId=${patientId}&medicationId=${med.id}">
                    ${med.name}
                  </a>
                </h3>
                <div>
                  <p>Dosis: ${med.dosage}</p>
                  <p>Fecha de inicio: ${new Date(med.start_date).toLocaleDateString("es-ES", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                  })}</p>
      ...
      this.scheduleBody.innerHTML = intakes
        .map((intake) => {
          ...
          const nearestPosology = posologies.reduce((nearest, posology) => {
            ...
            const currentDifference = timeDifferenceInMinutes(intakeHour, intakeMinute, posologyHour, posologyMinute);
          ...
          const nearestTimeFormatted = `${nearestTime.hour
            .toString()
            .padStart(2, "0")}:${(nearestTime.minute || 0).toString().padStart(2, "0")}`;

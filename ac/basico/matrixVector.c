/* Programa que multiplica una matrix por un vector de forma y=alfa*A*x
   ...
   El vector y se encuentra distribuido entre los procesos y se recoge al final
   en el 0*/
...
void matrizVectorLocal(float alfa, float *A, int lda, float *x, float *y,
                       int filas, int cols) {
    int i, j;
    for (i = 0; i < filas; i++) {
        for (j = 0; j < cols; j++) {
...
int main(int argc, char *argv[]) {
    ...
    // Parámetro 1 -> n
    // Parámetro 2 -> alfa
    // Parámetro 3 -> booleano que nos indica si se desea imprimir matrices y
    ...
    double t;
    MPI_Barrier(MPI_COMM_WORLD); // Barrera para garantizar una correcta medida
                                 // de tiempo
    t = MPI_Wtime();

    // Se envía A desde el proceso 0 a los diferentes procesos dividiendo por
    // filas con un scatter
    MPI_Scatter(A, filasBloque * n, MPI_FLOAT, localA, filasBloque * n,
    ...
    MPI_Gather(localy, filasBloque, MPI_FLOAT, y, filasBloque, MPI_FLOAT, 0,
               MPI_COMM_WORLD);
    t = MPI_Wtime() - t;
    ...
    if (!rank) {
        if (test) {
            ...
            // Solo el proceso 0 calcula el producto y compara los resultados
            // del programa secuencial con el paralelo
            ...
            for (i = 0; i < n; i++) {
                if (testy[i] != y[i]) {
                    errores++;
                    printf("\n Error en la posicion %d porque %f != %f", i,
                           y[i], testy[i]);
                }
            }
            printf(
    ...
    // Barrera para que no se mezcle la impresión del tiempo con la de los
    // resultados
    fflush(NULL);
    MPI_Barrier(MPI_COMM_WORLD);
    printf("\nProducto matriz vector con dimension %d y %d procesos: Tiempo de "
           "ejecucion del proceso %d fue %lf\n",
           n, numprocs, rank, t);

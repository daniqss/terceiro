...
class LoginView extends StatelessWidget {
    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          isWatch
              ? watchLoginView(patientCodeController, isWatch)
              : loginView(patientCodeController, isWatch),
        ],
      ),
    );
  }

  Widget watchLoginView(
      TextEditingController patientCodeController, bool isWatch) {
    return Center(
      child: Column(
          ...
          Consumer<LoginProvider>(
            builder: (context, loginProvider, child) {
              return LoginButton(
                ...
                onPressed: () async {
                  logInto(
                      loginProvider, patientCodeController, context, isWatch);
                },
              );
            },
          ),
  ...
  Widget loginView(TextEditingController patientCodeController, bool isWatch) {
    return Padding(
            ...
            children: [
              ...
              loginProvider.isLoading
                  ...
                  : LoginButton(
                      isWatch: isWatch,
                      onPressed: () async {
                        logInto(loginProvider, patientCodeController, context,
                            isWatch);
                      },
                    ),
              const Spacer(flex: 2),
  ...
  void showErrorMessage(
      BuildContext context, String message, Color color, bool isWatch) {
  ...
  void logInto(
    ...
    if (patientCode.isNotEmpty) {
      ...
      if (patient != null) {
        ...
        showErrorMessage(
            context, loginProvider.errorMessage, Colors.redAccent, isWatch);
      }
    } else {
      showErrorMessage(context, 'Por favor ingrese un código de paciente',
          Colors.black12, isWatch);
    }
  }
}
